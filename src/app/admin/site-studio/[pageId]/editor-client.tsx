...
export function SiteEditorClient({ initialPageData, initialPosts, pageId, tenantId }: SiteEditorClientProps) {
  ...
  const handleSave = () => {
    startTransition(async () => {
      const result = await savePage(tenantId, pageId, sections);
      toast({
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0] && tenantId && currentSectionIdForUpload) {
        const file = event.target.files[0];
        const sectionId = currentSectionIdForUpload;
        setIsUploading(sectionId);
        try {
            const storageRef = ref(storage, `tenants/${tenantId}/page_images/${Date.now()}_${file.name}`);
      ...
      <div className="flex-1 overflow-hidden">
        <div className="grid h-full grid-cols-1 md:grid-cols-[1fr_380px]">
            <div className="flex h-full items-center justify-center bg-muted/40 overflow-auto p-4 transition-all">
                <PagePreview sections={sections} previewMode={previewMode} posts={initialPosts} />
            </div>
